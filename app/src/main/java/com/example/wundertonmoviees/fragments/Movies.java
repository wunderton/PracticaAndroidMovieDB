package com.example.wundertonmoviees.fragments;
...
import com.example.wundertonmoviees.view.MainActivity;
import com.example.popularmovies.R;
import com.example.wundertonmoviees.adapter.MoviesAdapter;
import com.example.popularmovies.databinding.FragmentMoviesBinding;
import com.example.wundertonmoviees.model.Discover;
...
import com.example.wundertonmoviees.utils.DiscoverToMovie;
import com.example.wundertonmoviees.utils.PaginationScrollListener;
import com.example.wundertonmoviees.viewmodel.MainViewModel;
   ...
   @Override
    public void onViewCreated(@NonNull View view, @Nullable final Bundle savedInstanceState) {
       ...
       if(MainActivity.drawer==0) {
            getActivity().setTitle("Peliculas Nuevas");
        }
        else if(MainActivity.drawer==1)
        {
            getActivity().setTitle("Mejor Valoradas");
        }
        else if(MainActivity.drawer==2)
       {
          if(!MainActivity.genresLists.isEmpty()) {
              getActivity().setTitle("Genero: " + MainActivity.genresLists.get(MainActivity.selected).getName());
          }

       }
       else if(MainActivity.drawer==3)
       {
           getActivity().setTitle("Resultado de busqueda: "+MainActivity.queryM);
       }
        context=getContext();
    ...
    private void loadMore(int a, final int pages)
    {
        ...
        String ApiKey=RetrofitInstance.getApiKey();
        if(a==0) {
